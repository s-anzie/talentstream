...
import { addCandidateNote, deleteCandidateNote } from '@/lib/mock-api-services';
import { useToast } from '@/hooks/use-toast';
import { Skeleton } from '@/components/ui/skeleton';

export default function CandidateInternalNotesPage() {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const addedNote = await addCandidateNote(candidate.id, {
        ...
        // No jobId here as these are general notes on the candidate profile
      });
  ...
  // Filter notes that are general (no jobId)
  const generalNotes = candidate.internalNotes?.filter(note => !note.jobId) || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="flex items-center text-2xl text-primary"><FileText className="mr-3 h-7 w-7"/> Notes Internes Générales sur {candidate.name}</CardTitle>
          <CardDescription>
            Notes et commentaires de l'équipe de recrutement sur ce candidat, non spécifiques à une offre.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
            <div>
                <Textarea 
                    placeholder="Ajouter une note générale sur le candidat..." 
                    rows={4} 
                    value={newNote}
            ...
            <div className="space-y-4">
                ...
                {generalNotes.length > 0 ? (
                    generalNotes.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()).map(note => (
                    ...
                    <p className="text-sm text-muted-foreground">Aucune note générale pour ce candidat.</p>
                )}
            </div>
        </CardContent>
        <CardFooter>
            <p className="text-xs text-muted-foreground">Les notes générales sont utiles pour un suivi global du candidat.</p>
        </CardFooter>
      </Card>
    </div>
  );
