...
import { ArrowLeft, MessageSquare, Send, Paperclip, Smile, Loader2, Briefcase, User } from 'lucide-react';
...
export default function MessageCandidateForJobPage() {
  const params = useParams();
  const router = useRouter();
  const { toast } = useToast();
  const { user: recruiterUser } = useAuthStore(); // This is the recruiter

  const jobId = params.jobId as string;
  const candidateId = params.candidateId as string;
  // For messaging, a conversation ID might be derived from jobId and candidateId, or fetched.
  // For mock, let's construct a simple one.
  const conversationId = `job-${jobId}-cand-${candidateId}`;
  ...
  // Note: useFetchUserMessages expects a single conversationId. We might need a more specific hook or logic if conversations are more dynamic.
  ...
  const handleSendMessage = async (e: FormEvent) => {
    ...
    const messageToSend: Omit<UserMessage, 'id' | 'timestamp' | 'conversationId'> = {
        ...
        text: newMessage,
        isRead: false, // Will be true for the sender, false for receiver initially
    };

    try {
      // The mockApi.sendUserMessage is more generic, might need adjustment or a specific one for company-candidate messages
  ...
  return (
    <div className="space-y-6 flex flex-col h-[calc(100vh-var(--header-height,5rem)-4rem)]"> {/* Adjust height based on your layout */}
      <Button variant="outline" size="sm" onClick={() => router.push(`/dashboard/jobs/${jobId}/candidates/${candidateId}`)}>
      ...
      <Card className="shadow-lg flex-1 flex flex-col overflow-hidden">
        <CardHeader className="border-b">
          <CardTitle className="flex items-center text-2xl text-primary"><MessageSquare className="mr-3 h-7 w-7"/> Conversation avec {candidate.name}</CardTitle>
          <CardDescription>
            Concernant l'offre : <Link href={`/dashboard/jobs/${jobId}`} className="text-secondary hover:underline">{job.jobTitle}</Link>
          </CardDescription>
        </CardHeader>
        <ScrollArea className="flex-1 p-4 space-y-4 bg-muted/30">
            ...
            {!isLoadingMessages && messages?.map((msg) => (
                <div key={msg.id} className={`flex ${msg.senderId === recruiterUser?.id ? 'justify-end' : 'justify-start'}`}>
