...
import { CalendarIcon, Loader2, Lightbulb, Brain, Save, ArrowLeft, Edit } from "lucide-react"; // Added Edit
...
import { updateDashboardJob, fetchDashboardJobDetails } from "@/lib/mock-api-services";
...
export default function EditJobPage() {
  ...
  useEffect(() => {
    if (jobId) {
      ...
      const loadJobData = async () => {
        try {
          const jobData = await fetchDashboardJobDetails(jobId);
          if (jobData) {
            form.reset({
              ...jobData,
          ...
          toast({ title: "Erreur de Chargement", description: "Impossible de charger les détails de l'offre.", variant: "destructive" });
        } finally {
  ...
  async function onSubmit(data: JobPostingFormData) {
    ...
    try {
      await updateDashboardJob(jobId, data);
      toast({ title: "Offre Mise à Jour !", description: `L'offre "${data.jobTitle}" a été mise à jour.` });
  ...
  if (isLoadingJob) {
    return (
      <Card className="shadow-xl">
        <CardHeader><CardTitle className="text-3xl font-bold text-primary flex items-center"><Loader2 className="mr-3 h-8 w-8 animate-spin" /> Chargement de l'offre...</CardTitle></CardHeader>
        <CardContent className="space-y-6">
          <Skeleton className="h-10 w-full" /> <Skeleton className="h-24 w-full" />
    ...
    <Card className="shadow-xl">
      <CardHeader>
        <div className="flex justify-between items-center">
            <CardTitle className="text-3xl font-bold text-primary flex items-center"><Edit className="mr-3 h-8 w-8" /> Modifier l'Offre</CardTitle>
            <Button variant="outline" asChild><Link href={`/dashboard/jobs/${jobId}`}><ArrowLeft className="mr-2 h-4 w-4" /> Retour à l'offre</Link></Button>
        </div>
        <CardDescription>Mettez à jour les détails de l'offre d'emploi.</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <FormField control={form.control} name="applicationDeadline" render={({ field }) => (
              <FormItem className="flex flex-col"><FormLabel className="text-lg">Date Limite Candidature (Optionnel)</FormLabel><Popover>
                <PopoverTrigger asChild><FormControl><Button variant={"outline"} className={cn("w-[240px] pl-3 text-left font-normal",!field.value && "text-muted-foreground")}>
                  {field.value ? format(new Date(field.value), "PPP", { locale: fr }) : <span>Choisissez une date</span>}<CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
                </Button></FormControl></PopoverTrigger>
                <PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={field.value ? new Date(field.value) : undefined} onSelect={field.onChange} disabled={(date) => date < new Date(new Date().setDate(new Date().getDate() -1))} initialFocus locale={fr}/></PopoverContent>
              </Popover><FormMessage /></FormItem>
            )}/>
            <div className="flex justify-end pt-6">
              <Button type="submit" size="lg" className="bg-primary hover:bg-primary/90" disabled={form.formState.isSubmitting}>
                {form.formState.isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{form.formState.isSubmitting ? "Sauvegarde..." : "Sauvegarder Modifications"} <Save className="ml-2 h-5 w-5"/>
              </Button>
            </div>
          </form>
        </Form>
